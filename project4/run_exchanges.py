...
def exchange(sess):
    ...
    V_1 = sess.execute(
        "SELECT ACCOUNT.BALANCE FROM ACCOUNT WHERE ACCOUNT.ID =:A_1", {
            "A_1": A_1
        }).scalar()
    V_2 = sess.execute(
        "SELECT ACCOUNT.BALANCE FROM ACCOUNT WHERE ACCOUNT.ID =:A_2", {
            "A_2": A_2
        }).scalar()
    ## 2. Swap the value
    sess.execute("UPDATE ACCOUNT SET BALANCE =:V_2 WHERE ACCOUNT.ID =:A_1", {
        "V_2": V_2,
        "A_1": A_1
    })
    sess.execute("UPDATE ACCOUNT SET BALANCE =:V_1 WHERE ACCOUNT.ID =:A_2", {
        "V_1": V_1,
        "A_2": A_2
    })
